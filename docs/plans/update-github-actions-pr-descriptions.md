...
5. `codeql-analysis.yml` запускается ещё и по расписанию
   (`cron: '0 0 * * 2'`, вторник 00:00 UTC). Такой прогон — не проверка
   этого PR: он появится в Actions отдельно и не должен читаться как
   результат PR-2.
