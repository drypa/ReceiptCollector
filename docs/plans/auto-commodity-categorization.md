...
- ИИ возвращает JSON `{"category": "<имя значения CommodityCategory>"}` (например, `{"category": "Food"}`) строго из `CommodityCategory` (актуальный состав перечисления — см. ADR 010; в промт попадают все значения, кроме `Undefined`); числовой формат `{"category_id": N}` отклонён решением заказчика; частичный успех (UC-4).
...
- Промт генерируется из `CommodityCategoryHelper.GetAll()` без `Undefined` (см. ADR 009, п. 4) — **все существующие значения перечисления** (актуальный состав после ADR 010, число в документе не фиксируется). Требования к промту (ADR 010, решение E1):
...
- Файл `Analytics/src/ReceiptCollector.Analytics.Infrastructure/Modules/Commodities/CommodityCategorizationService.cs`:
  - Загрузка чека через `IReceiptReadService.GetByIdAsync(userId, receiptId)` (owner-scoped, `404` на уровне эндпоинта).
   ...
   - Сохранение: для каждого `SaveCategoryUpdate` — `ICommodityRepository.UpdateCategoryAsync(commodityId, category)` (**правка контракта: nullable-параметр `UpdateCategoryAsync(Guid, CommodityCategory?, ct)`** — текущий не-nullable метод не умеет сброс; при `null` реализация пишет `category_id = NULL`/`category_name = NULL`, см. ADR 009, п. 5) + `ICategoryAssignmentRepository.UpsertAsync(name, normalizedName, category)` для позиций с назначенной категорией.
...
- DTO-доработка: добавить `Id` (GUID `commodities.id`) в `ReceiptItemDto` (`Analytics/src/ReceiptCollector.Analytics.Application/Modules/Receipts/Models/ReceiptDetailsDto.cs`) и в маппинг `ReceiptReadService.ToReceiptDto` (`Analytics/src/ReceiptCollector.Analytics.Infrastructure/Modules/Receipts/ReceiptReadService.cs`, EF-проекция формирует DTO) — без этого невозможно сохранять категории по `commodityId`. `ReceiptEntity.MapToDomain()` строит доменный `Receipt` и DTO не затрагивает.
...
- Консистентность справочника (каждый член перечисления имеет отображаемое имя, `Food` = «Прочая еда», группировка UI) — покрыта отдельным `CommodityCategoryTests.cs` (создан в задаче «Расширение справочника категорий товаров», ADR 010); при правках справочника сверяться с ним.
