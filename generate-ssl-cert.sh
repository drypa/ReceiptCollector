...
#
# Результат кладётся в каталог из SSL_CERTS_PATH (из .env), либо по умолчанию
# в ssl/ рядом со скриптом:
...
# 2. Пути
# ---------------------------------------------------------------------------
# Куда генерировать сертификаты:
#   - если в .env задана SSL_CERTS_PATH — в неё;
#   - иначе — ./ssl рядом со скриптом (прежнее поведение).
if [ -n "${SSL_CERTS_PATH:-}" ]; then
    ssl_path="$SSL_CERTS_PATH"
    # Относительный путь интерпретируем относительно каталога скрипта,
    # чтобы поведение не зависело от текущего рабочего каталога
    case "$ssl_path" in
        /*) ;;
        *)  ssl_path="${SCRIPT_DIR}/${ssl_path}" ;;
    esac
else
    ssl_path="${SCRIPT_DIR}/ssl"
fi

# Нормализация: убираем завершающие слэши, кроме корня "/"
while [ "$ssl_path" != "/" ] && [[ "$ssl_path" == */ ]]; do
    ssl_path="${ssl_path%/}"
done
